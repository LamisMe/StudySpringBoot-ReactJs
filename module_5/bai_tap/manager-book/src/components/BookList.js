        ...
        <tbody>
            {books.map(item => (              
                <tr key={item.id}>
                    ...
                    <td>{item.quantity}</td>
                    <td>
                    <Link className="btn btn-sm btn-primary rounded-0 me-3" type="button"
                                             to={`/update/${item.id}`}>Edit
                                    </Link>
                    <button type="button" data-bs-toggle="modal" data-bs-target="#exampleModal"
                                            className="btn btn-sm btn-danger rounded-0"
                                            onClick={() => handlerBook(item)}>Delete
                                    </button>
                    </td>
                </tr>
            ))}
                            ...
                            <div className="modal-header justify-content-center">
                                <div>
                                    <h1 className="modal-title text-danger fs-4" id="exampleModalLabel">DELETE
                                        BOOK</h1>
                                    <h5 className="modal-title text-danger fw-bold">{bookDelete.title}</h5>
                                </div>
                            </div>
